...
public interface PlantRepository {
	...
	@Query("SELECT p FROM Plant AS p WHERE p.id NOT IN (SELECT po.plant FROM PurchaseOrder po WHERE po.startDate >= :startDate and po.startDate <= :startDate and po.endDate <= :endDate and po.endDate >= :endDate and po.status != :pendingConfirmation)")
